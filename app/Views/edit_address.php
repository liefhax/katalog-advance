...
<div class="py-8 px-4">
    <div class="max-w-7xl mx-auto space-y-8 animate-fade-in">
        ...
        <div class="bg-white shadow-sm border-t-4 border-black overflow-hidden">
            ...
            <form action="<?= base_url('profile/addresses/update/' . $address['id']) ?>" method="post" class="p-6 space-y-6">
                <?= csrf_field() ?>
                ...
                <div>
                    ...
                    <input type="text"
                           id="label"
                           name="label"
                           value="<?= esc($address['label'] ?? old('label')) ?>"
                ...
                <div>
                    ...
                    <input type="text"
                           id="recipient_name"
                           name="recipient_name"
                           value="<?= esc($address['recipient_name'] ?? old('recipient_name')) ?>"
                           class="w-full px-3 py-2 border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200"
                ...
                <div>
                    ...
                    <input type="tel"
                           ...
                           value="<?= esc($address['recipient_phone'] ?? old('recipient_phone')) ?>"
                           class="w-full px-3 py-2 border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200"
                ...
                <div>
                    ...
                    <textarea id="street_address"
                              ...
                              required><?= esc($address['street_address'] ?? old('street_address')) ?></textarea>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="hidden" name="province_kode" id="province_kode" value="<?= esc($address['province_kode'] ?? old('province_kode')) ?>">
                        <input type="hidden" name="province_name" id="province_name" value="<?= esc($address['province_name'] ?? old('province_name')) ?>">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="hidden" name="city_kode" id="city_kode" value="<?= esc($address['city_kode'] ?? old('city_kode')) ?>">
                        <input type="hidden" name="city_name" id="city_name" value="<?= esc($address['city_name'] ?? old('city_name')) ?>">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="hidden" name="subdistrict_kode" id="subdistrict_kode" value="<?= esc($address['subdistrict_kode'] ?? old('subdistrict_kode')) ?>">
                        <input type="hidden" name="subdistrict_name" id="subdistrict_name" value="<?= esc($address['subdistrict_name'] ?? old('subdistrict_name')) ?>">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="text"
                           id="postal_code"
                           name="postal_code"
                           value="<?= esc($address['postal_code'] ?? old('postal_code')) ?>"
                           class="w-full px-3 py-2 border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200"
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    try {
        ...
        provinces.forEach(prov => {
            const selected = (prov.kode === '<?= esc($address['province_kode'] ?? '') ?>') ? 'selected' : '';
            provinceSelect.innerHTML += `<option value="${prov.nama}" data-id="${prov.kode}" ${selected}>${prov.nama}</option>`;
        });

        // Jika sudah ada data provinsi, load kota
        if ('<?= esc($address['province_kode'] ?? '') ?>') {
            await loadCities('<?= esc($address['province_kode'] ?? '') ?>');
        }
    } catch (e) {
    ...
    provinceSelect.addEventListener('change', async function () {
        ...
        document.getElementById('province_kode').value = provinceCode;
    ...
    citySelect.addEventListener('change', async function () {
        ...
        document.getElementById('city_kode').value = cityCode;
        document.getElementById('city_name').value = cityName;

        subdistrictSelect.innerHTML = '<option value="">Memuat...</option>';
        subdistrictSelect.disabled = true;

        await loadSubdistricts(cityCode);
    });
    ...
    subdistrictSelect.addEventListener('change', function () {
        ...
        document.getElementById('subdistrict_kode').value = subdistrictCode;
    ...
    async function loadCities(provinceCode) {
        try {
            ...
            cities.forEach(city => {
                const selected = (city.kode === '<?= esc($address['city_kode'] ?? '') ?>') ? 'selected' : '';
                citySelect.innerHTML += `<option value="${city.nama}" data-id="${city.kode}" ${selected}>${city.nama}</option>`;
            });
            citySelect.disabled = false;

            // Jika sudah ada data kota, load kecamatan
            if ('<?= esc($address['city_kode'] ?? '') ?>') {
                await loadSubdistricts('<?= esc($address['city_kode'] ?? '') ?>');
            }
        } catch (e) {
    ...
    // Helper function to load subdistricts
    async function loadSubdistricts(cityCode) {
        try {
            ...
            subdistrictSelect.innerHTML = '<option value="">Pilih Kecamatan...</option>';
            districts.forEach(d => {
                const selected = (d.kode === '<?= esc($address['subdistrict_kode'] ?? '') ?>') ? 'selected' : '';
                subdistrictSelect.innerHTML += `<option value="${d.nama}" data-id="${d.kode}" ${selected}>${d.nama}</option>`;
            });
